...
class FIRStack(nn.Module):
    """
    1D FIR(홀수길이) 두 번으로 low/high 추출 (세로 kx1 → 가로 1xk)
      - 입력: x [B,C,H,W] (C=3이면 Y로 변환 후 단일 채널 처리)
      - 출력: [B,2,H,W]  (채널 0 = low, 채널 1 = high)
      - 동작: MATLAB filter2(h, x, 'same')와 동일한 '상관(correlation)' (커널 뒤집기 X)
      - 패딩: 'same' 유지 (각 단계에서 half padding 적용)
      - 범위: 결과는 0~1로 clamp
    """
    ...
    def __init__(self, c1: int, c2: int = 2):
        ...
        assert low1d.ndim == 1 and high1d.ndim == 1
        assert len(low1d) % 2 == 1 and len(high1d) % 2 == 1
        assert len(low1d) == len(high1d), "low/high 커널 길이가 같아야 합니다."
        k = len(low1d)

        # 세로(kx1) 2채널 스택 → [2,1,k,1]
        kv = torch.stack([low1d.view(1, -1, 1), high1d.view(1, -1, 1)], dim=0)
        # 가로(1xk) 2채널 스택 → [2,1,1,k]  (groups=2에서 각 채널별 적용)
        kh = torch.stack([low1d.view(1, 1, -1), high1d.view(1, 1, -1)], dim=0)

        # 버퍼 등록(상관이므로 flip 없음)
        self.register_buffer("kv", kv)  # [2,1,k,1]
        self.register_buffer("kh", kh)  # [2,1,1,k]
        self.khalf = k // 2
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # RGB -> Y (상관 연산용 단일 채널)
        if x.shape[1] == 3:
            y = (x * self.rgb2y.to(dtype=x.dtype, device=x.device)).sum(1, keepdim=True)
        else:
            y = x

        # 채널라스트가 conv2d에 유리한 경우가 많음
        y = y.contiguous(memory_format=torch.channels_last)

        # 버퍼 dtype/device 맞추기
        kv = self.kv.to(dtype=y.dtype, device=y.device)
        kh = self.kh.to(dtype=y.dtype, device=y.device)

        # 패딩 계산
        pad_v = self.khalf
        pad_h = self.khalf

        # 1) 세로: 1채널 → 2채널 (low/high를 동시에)
        y2 = F.conv2d(y, kv, bias=None, stride=1, padding=(pad_v, 0))  # [B,2,H,W]

        # 2) 가로: groups=2로 각 채널별 해당 커널만 적용
        out = F.conv2d(y2, kh, bias=None, stride=1, padding=(0, pad_h), groups=2)  # [B,2,H,W]
